...
interface ChevronDownIconProps { isOpen: boolean; }
const ChevronDownIcon = ({ isOpen }: ChevronDownIconProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" className={`w-4 h-4 ml-auto transition-transform duration-200 ${isOpen ? "rotate-180" : "rotate-0"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
...
const ProfileDropdown: React.FC = () => {
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center space-x-2 p-1 pr-3 rounded-full transition duration-200 hover:bg-purple-200/50" style={{ backgroundColor: "#f3e8ff" }} aria-expanded={isOpen}>
        <div className="w-8 h-8 rounded-full bg-purple-300 flex items-center justify-center text-purple-800"><UserIcon /></div>
        <span className="font-semibold text-purple-800 text-sm hidden sm:inline">Xin chào, {user?.name || 'Admin'}</span>
      </button>
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-1 border border-zinc-200 z-10">
          <button onClick={() => { setIsOpen(false); router.push(getProfileUrl()); }} className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-600">Cập nhật thông tin</button>
          <button onClick={() => { setIsOpen(false); router.push(getChangePasswordUrl()); }} className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-600">Đổi mật khẩu</button>
          <div className="border-t border-zinc-100 my-1"></div>
          <button onClick={(e) => { e.preventDefault(); setShowLogoutConfirm(true); setIsOpen(false); }} className="w-full text-left block px-4 py-2 text-sm text-red-600 hover:bg-red-50">← Đăng xuất</button>
        </div>
      )}
      {showLogoutConfirm && <LogoutConfirmationModal isOpen={showLogoutConfirm} onConfirm={handleLogoutConfirm} onCancel={() => setShowLogoutConfirm(false)} />}
    </div>
  );
};
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl">
        ...
        <div className="flex gap-3 justify-end">
          <button type="button" onClick={onCancel} className="px-4 py-2 text-gray-600 hover:text-gray-800 transition">Hủy</button>
          <button type="button" onClick={onConfirm} className="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 transition">Đăng xuất</button>
        </div>
      </div>
    </div>
  );
...
interface NavItem { name: string; href: string; submenu?: NavItem[]; }
const AdminSidebar: React.FC = () => {
  const navItems: NavItem[] = [
    { name: "Trang chủ", href: "/admin" },
    { name: "Quản lý tài khoản", href: "/admin/accounts", submenu: [{ name: "Giáo viên", href: "/admin/accounts/teachers" }, { name: "Học sinh", href: "/admin/accounts/students" }] },
  ...
  return (
    <aside className="w-64 bg-white border-r border-zinc-200 h-screen fixed top-0 left-0 z-50 shadow-xl overflow-y-auto">
      <div className="flex items-center space-x-2 px-4 py-3 border-b border-zinc-200 bg-white">
        <a href="/" className="text-xl font-black" style={{ color: LOGO_TEXT_COLOR }}>QuizzZone</a>
      </div>
      <nav className="text-xs font-medium text-zinc-700">
        <div className="h-1 border-b border-zinc-200" style={{ backgroundColor: "#eee" }}></div>
        {navItems.map(item => {
          ...
          if (hasSubmenu) {
            return (
              <div key={item.name} className="border-b border-zinc-200">
                <button onClick={() => handleToggleSubmenu(item.name)} className={`flex items-center w-full px-4 py-2 text-xs transition duration-150 text-left ${isCurrentActive ? "bg-zinc-100 font-semibold text-purple-700" : "hover:bg-zinc-50"}`}>
                  <span>{item.name}</span>
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}>
                  {item.submenu?.map(subItem => {
                    ...
                    return <a key={subItem.name} href={subItem.href} className={`flex items-center w-full pl-6 pr-4 py-1.5 text-xs transition duration-150 ${subIsActive ? "bg-zinc-200 font-semibold text-purple-700" : "hover:bg-zinc-100"}`}><span>{subItem.name}</span></a>;
                  })}
                </div>
              </div>
            );
          }
          return <a key={item.name} href={item.href} className={`flex items-center w-full px-4 py-2 text-xs transition duration-150 border-b border-zinc-200 ${isCurrentActive ? "bg-zinc-100 font-semibold text-purple-700" : "hover:bg-zinc-50"}`}><span>{item.name}</span></a>;
        })}
      </nav>
    </aside>
  );
...
const AdminTopBar = () => (
  <header className="w-full border-b border-zinc-200 bg-white sticky top-0 z-40 shadow-sm">
    <div className="flex items-center justify-between px-4 py-2 md:py-3">
      <div className="flex items-center"><a href="/admin" className="text-2xl font-black tracking-tighter" style={{ color: LOGO_TEXT_COLOR }}>QuizzZone</a></div>
  ...
  return (
    <AdminAuthGuard>
      <div className="flex flex-col min-h-screen bg-gray-50">
        <AdminSidebar />
        <AdminTopBar />
        <div className="lg:ml-64 flex flex-col flex-1 transition-[margin-left] duration-300">
